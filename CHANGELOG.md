...
### Added
- **Disabled-by-default local DB compile harness stub gate (T023):** Added [docs/runtime/LOCAL_DB_COMPILE_HARNESS.md](docs/runtime/LOCAL_DB_COMPILE_HARNESS.md), [config/local-db-compile-harness.json](config/local-db-compile-harness.json), `scripts/runtime/local-db-compile-harness-stub.mjs`, `scripts/runtime/validate-local-db-compile-harness.mjs`, and package scripts `runtime:db-compile-harness:stub` / `runtime:validate:db-compile-harness` with enforced `enabled=false` and `execution_allowed_now=false` (no SQL execution, no Docker, no psql, no Supabase CLI, no DB/network execution).
